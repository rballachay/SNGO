...
using MathProgBase, Distributions
...
function copyVarsCons(model::Model)
...
"""
This funcion is deprecated since updating JuMP 2020-06-15
...
getparent(m::JuMP.Model)       = m.ext[:parent]
getchildrenDict(m::JuMP.Model)     = m.ext[:childrenDict]
...
function getchildren(m::JuMP.Model)
    if haskey(m.ext, :children)
        return m.ext[:children]
    else
        return []
    end
end
...
function getNode(m::JuMP.Model, modelname::String)
    if !haskey(m.ext[:childrenDict], modelname)
        error("No model with name $modelname")
    elseif m.ext[:childrenDict][modelname] === nothing
        error("Multiple models with name $modelname")
    else
        return m.ext[:childrenDict][modelname]
    end
end
...
function registermodel(m::JuMP.Model, modelname::String, value::JuMP.Model)
    if haskey(m.ext[:childrenDict], modelname)
        m.ext[:childrenDict][modelname] = nothing # indicate duplicate
        error("Multiple models with name $modelname")
    else
        m.ext[:childrenDict][modelname] = value
    end
end
...
macro addNode(m, node)
    if isa(node, Symbol)
       return quote       	    
    	   if haskey($(esc(node)).ext, :parent)
	        ($(esc(node))).ext[:parent] = $(esc(m))
    	   else
		$(esc(node)).ext[:children] = JuMP.Model[]
        $(esc(node)).ext[:parent] = $(esc(m))
        $(esc(node)).ext[:childrenDict] = Dict{Symbol, JuMP.Model}()
           end
	   push!(($(esc(m))).ext[:children], $(esc(node)))
...
function RandomStochasticModel(createModel, nscen=100, nfirst=5, nparam=5, rdl=0.0, rdu=2.0, adl=-10, adu=10 )
	...
	if i == 1
        ...
        if (typeof(nparam) == Int64) && nmodified < floor(nparam/2)
            for c = 1:length(node.quadconstr)
                ...
                if length(findall((in)(firstVarsId),varsId))  == length(varsId)
                    continue
                end
	...
	=#
	    ...
	    if length(findall((in)(firstVarsId),v)) != 0
	        continue
	    end
...
function extensiveSimplifiedModel(P::JuMP.Model)
    ...
    # Reference for MOI.get: MathOptInterface/C3lip/src/Utilities/model.jl
    # Reference for ListOfConstraints: MathOptInterface/C3lip/src/functions.jl
    cons = all_constraints(P,AffExpr,MOI.GreaterThan{Float64})
    print(cons)
    
    for (F,S) in MOI.get(P, MOI.ListOfConstraints())
        print(F.terms)
        for index in MOI.get(P, MOI.ListOfConstraintIndices{F,S}())
    end
    end
    for c in MOI.get(P,MOI.ListOfConstraints())
    print((MOI.get(P,MOI).ListOfConstraintIndices{F,S}())[2])
    ##### This returns the following
    # Tuple{DataType,DataType}[(ScalarAffineFunction{Float64}, EqualTo{Float64}), (SingleVariable,  GreaterThan{Float64}), (SingleVariable, LessThan{Float64})]
...
function addnodeSimplifiedmodel!(m::JuMP.Model,node::JuMP.Model, nodename, scen)
	...
	firstVarsId = node.ext[:firstVarsId] 
        ...
        v_map = Array{Int}(undef,num_vars)		#this dict will map linear index of the node model variables to the new model JuMP variables {index => JuMP.Variable}
        #add the node model variables to the new model
        for i = 1:num_vars
	    first = findall((in)(i),firstVarsId);
	    if length(first) == 0  
                x = JuMP.@variable(m)            #create an anonymous variable
	...
	push!(m.ext[:v_map], v_map)
	    ...
	    if length(findall((in)(firstVarsId),varsId))  == length(varsId) && scen != 1
	        continue
	    end
...
function copyModel(P::JuMP.Model)
    ...
    m = Model()
    m,ref = copyVarsCons(P)
    
    # In copy model, there are the following requirements:
        # - Variables -> Accounted for in obj_dict
        # - Constraints -> Accounted for in obj_dict
        # - Objective -> Copied over with the model definition
