...
StoSol(nscen) = StoSol(Float64[], 0, Array{Array{Float64}}(undef,nscen), Array{Float64}(undef,nscen))
...
function parselinear(expr)
    ...
    #aff = AffExpr(Float64[],Float64[],0.0)
    constant = 0
    varId = -1
    coeff = 0
...
function parseAff(m, expr)
	...
	if varid != -1
	    aff.vars = [Float64(m, varid)]
	    aff.coeffs = [coeff]
	end
	=#
	#println(varid, coeff, constant)
	if varid == -1
	    aff = AffExpr(Float64[],Float64[],constant)
	else
	    aff = AffExpr([Float64(m, varid)],[coeff],constant)
	end    
	return aff
    end
    if typeof(expr) == Expr
       ...
       if expr.args[1] == :(*)       	    
		...
		elseif islinear(expr.args[i])
		    ...
		    if length(quad.qvars1) == 0
		       push!(quad.qvars1, Float64(m, varid))
		    else
		       push!(quad.qvars2, Float64(m, varid))	
		    end   
                    continue
                else
                    return false
...
function _addcon!(m, expr, output = :(aff))
	...
	if n >= 2
	    ...
	    if islinear(ex1) && islinear(ex2)
	        ...
	        coeff = coeff1*coeff2
	        var1 = Float64(m, varid1)
	        var2 = Float64(m, varid2)
...
=#
function fixVar(v::Float64, val::Number)
